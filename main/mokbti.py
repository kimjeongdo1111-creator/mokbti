...
plt.rcParams['font.sans-serif'] = [KOREAN_FONT, 'NanumGothic', 'Gulim', 'sans-serif']
...
def plot_voice_spectrum_bar(valid_pitches, bti_res):
    """모바일 최적화 주파수 스펙트럼 내 음역대 영역 분포 바 차트"""
    ...
    zones = [
        ("동굴저음형\n(< 95Hz)", x_min, th["th_asmr"], "#8B5CF6", 0.16),
        ("ASMR형\n(95-135Hz)", th["th_asmr"], th["th_narration"], "#A855F7", 0.18),
        ("내레이션형\n(135-190Hz)", th["th_narration"], th["th_vitamin"], "#3B82F6", 0.20),
        ("비타민형\n(190-255Hz)", th["th_vitamin"], th["th_high"], "#10B981", 0.20),
        ("하이톤형\n(> 255Hz)", th["th_high"], x_max, "#06B6D4", 0.18),
    ]

    for name, z_start, z_end, color, alpha in zones:
        ax.axvspan(z_start, z_end, ymin=0.15, ymax=0.85, color=color, alpha=alpha)
        ax.axvline(z_start, color='#CBD5E1', linestyle='--', linewidth=1.0)
        mid = (z_start + z_end) / 2
        ax.text(mid, 0.50, name, color='#1E293B', fontsize=10.5, fontweight='bold', fontfamily=KOREAN_FONT,
                ha='center', va='center', bbox=dict(boxstyle='round,pad=0.35', facecolor='#FFFFFF', edgecolor='#CBD5E1', linewidth=1.0))

    ax.axvline(x_max, color='#CBD5E1', linestyle='--', linewidth=1.0)
    ...
    rect = patches.FancyBboxPatch(
        (min_f0, 0.18), max_f0 - min_f0, 0.64,
        boxstyle="round,pad=0.03", linewidth=2.5, edgecolor='#E11D48', facecolor='#FDA4AF', alpha=0.50, zorder=4
    )
    ax.add_patch(rect)

    note_clean = bti_res['note_mean'].replace('♯', '#')
    ax.plot([mean_f0, mean_f0], [0.05, 0.95], color='#D97706', linewidth=3.5, zorder=5)
    ax.scatter([mean_f0], [0.50], color='#D97706', s=200, zorder=6, edgecolor='#FFFFFF', linewidth=2.5)
    ax.text(mean_f0, 1.02, f"평균 기본주파수: {mean_f0:.1f}Hz ({note_clean})",
            color='#92400E', fontsize=11.5, fontweight='bold', fontfamily=KOREAN_FONT, ha='center', va='bottom',
            bbox=dict(boxstyle='round,pad=0.35', facecolor='#FEF3C7', edgecolor='#F59E0B', linewidth=1.5))

    ax.text(min_f0, 0.05, f"최저 {min_f0:.0f}Hz", color='#BE123C', fontsize=10.5, fontweight='bold', fontfamily=KOREAN_FONT, ha='center', va='top')
    ax.text(max_f0, 0.05, f"최고 {max_f0:.0f}Hz", color='#BE123C', fontsize=10.5, fontweight='bold', fontfamily=KOREAN_FONT, ha='center', va='top')
    ...
    ax.set_xticks(ticks)
    ax.set_xticklabels([f"{t}Hz" for t in ticks], fontsize=10.5, color='#334155', fontweight='bold', fontfamily=KOREAN_FONT)
    ax.set_xlabel("주파수 대역 (Frequency Hz)", color='#475569', fontsize=11, labelpad=8, fontweight='bold', fontfamily=KOREAN_FONT)
    ax.tick_params(colors='#94A3B8', bottom=True, length=5)
